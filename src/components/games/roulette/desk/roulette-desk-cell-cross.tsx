...
const RouletteDeskCellCross = memo(
  ({
    value,
    onClick,
    setHoveredRange,
    ...
    return (
      <button
        ...
        className={cn(
          value,
          'relative z-20 flex translate-x-[-33%] translate-y-[-33%] items-center justify-center p-2',
        )}
      >
        {chips?.length
          ? chips.map((chip, index) => {
              ...
              return (
                <div
                  key={chip + index}
                  className="absolute left-0 top-0 z-20 flex translate-x-[-33%] translate-y-[-33%] items-center justify-center rounded-full border-[3px] border-transparent"
                  style={{
                    left: `${index * 3}px`,
                  }}
                >
...
export default RouletteDeskCellCross;
RouletteDeskCellCross.displayName = 'RouletteDeskCellCross';
